...
def DW(epsilon):
    ...
    DW_stat = sum([(i_t - i_t_1)**2 for i_t, i_t_1 in zip(epsilon[1:], epsilon[:-1])]) / \
...
def ACF(series, s):
    ...
    if denominator == 0:
        return 0.0
    numerator = sum([(series.iloc[i] - mean)*(series.iloc[i - s] - mean) for i in range(s, N)])
...
class InputReceiver:
        ...
        try:
            with open(path, 'r') as f:
                if key == 1:
                    ...
                    return [float(i.partition('=')[2].strip()) for i in f.readlines() if i.partition('=')[2].strip()]
    ...
    def part_one(self, file_path: str = None):
        if file_path:
            ...
            DW_series = self.read_file('Шлях до файлу з даними для розрахунку статистики Дарбіна-Уотсона', key=1, file_path_override=file_path)
        else:
            # Для ручного введення
            DW_series = self.read_file('Шлях до файлу з даними для розрахунку статистики Дарбіна-Уотсона', key=1)
        return DW_series
    ...
    def load_data(self):
        ...
        pathes = ['RTStl.txt', 'RTSog.txt', 'RTSmm.txt', 'RTSin.txt', 'RTSfn.txt', 'RTSeu.txt', 'RTScr.txt']
        ...
        for path in pathes:
            try:
                ...
                print(f"Помилка: Файл {dir + path} не знайдено. Перевірте шляхи до файлів даних.")
...
def descriptive_analysis(series):
    ...
    for stat, val in stats_to_display:
        ...
        ax.text(1.5, counter, f'{val:.6f}' if isinstance(val, (int, float)) else str(val).ljust(10), transform=ax.transAxes, fontsize=9)
        counter -= 0.05
...
def stats_report(y_true, y_pred, n_params, coefs, p, name=''):
    ...
    print(*[f'a{i} = {coef:.3f}' for i, coef in enumerate(ar_coefs[1:], 1)], sep='; ', end='')
    ...
    print('AIC (Akaike info criterion):'.ljust(30), f'{IKA(y_true, y_pred, n=n_params + 1):.6f}'.ljust(15)) 
...
def LS(y, X):
    ...
    y_eff = np.array(y)[X.shape[0]:] if len(y) > X.shape[0] else np.array(y)[:X.shape[0]]
    ...
    try:
        ...
        print('Матриця Х виявилася виродженою')
         ...
         print(f'Помилка розмірності при МНК: {e}. X.shape={X.shape}, y_eff.shape={y_eff.shape}')
    finally:
        return res
    ...
    if len(coeffs) != (1 + p + q):
        ...
        if len(coeffs) == (p + q):
            coeffs = np.concatenate([[0], coeffs])
             ...
             print(f"Помилка: Некоректна кількість коефіцієнтів ({len(coeffs)}) для ARMA({p},{q}). Очікується {1+p+q}.")
             return np.zeros(N)
...
def resid_approach(series, p, w_size, dummy=[]):
    ...
    if not dummy:
        # Звіт для AR(p)
        stats_report(series, y_pred_full, n_params=p, coefs=AR_coefs, p=p, name=f'AR({p})')
    ...
    if not dummy:
        ...
        try:
            ...
            md_ar_coefs = [arma_model.params.iloc[0]] + [arma_model.params.iloc[i] for i in range(1, p + 1)]
            md_ma_coefs = [arma_model.params.iloc[i] for i in range(p + 1, p + q + 1)]
            
            # Коефіцієнти для stats_report: [const, a1..ap, b1..bq]
            stats_report(series.iloc[arma_model.loglikelihood_mle.get_start() -1 : arma_model.loglikelihood_mle.get_end()], 
                         arma_pred, n_params=p + q, coefs=md_ar_coefs + md_ma_coefs, 
                         p=p, name=f'ARMA({p}, {q}) із бібліотеки statsmodels (Підхід №1.2.1)')
    ...
    sma_res = {'y_true': series_eff, 'y_pred': sma_pred_eff, 'n_params': p + q, 'coefs': ARMA_sma_coefs, 'p': p, 
                'name': f'ARMA({p}, {q}) із застосуванням власного простого КС (розмір вікна {w_size}) (Підхід №1.2.3)'}
    ema_res = {'y_true': series_eff, 'y_pred': ema_pred_eff, 'n_params': p + q, 'coefs': ARMA_ema_coefs, 'p': p, 
...
def original_series_approach(series, p, w_size, ma=sma, dummy=[]):
    ma_type = {sma: 'власного простого КС', 
               ema: 'власного експоненційного КС'}
    print('\n\n' + '='*80)
    ...
    ma_part_values = np.sum([b_j * X_ma_part[:, i] for i, b_j in enumerate(ma_coefs, 1)], axis=0)
    ...
    formula_res = {'y_true': series_eff, 'y_pred': pred_eff, 'n_params': p + q, 'coefs': coefs, 'p': p, 
    ...
    smlt_res = {'y_true': series_eff, 'y_pred': pred_smlt_eff, 'n_params': p + q, 'coefs': smlt_coefs, 'p': p, 
...
def adjust_ARMA(series):
    ...
    if result[1] > ALPHA:
        ...
        if result_diff[1] > ALPHA:
            print('Ряд не стаціонарний навіть після взяття перших різниць. Подальший аналіз може бути некоректним.')
        else:
    ...
    if p == 0: p = 1 # Мінімум AR(1), якщо кореляції немає
    print(f'\nПрипущення: p = {p} (по останньому значущому лагу)')


    while True:
        N_orig = len(series)
        W_SIZE = InputReceiver().validate_input('Введіть розмір вікна (N) для КС', 'очікується натуральне число менше розміру ряду', 
        ...
        orig_dummy = []
        for ma in ma_types:
            for res in original_series_approach(series=series, p=p, w_size=W_SIZE, ma=ma, dummy=orig_dummy):
                if res:
                    stats_report(**res)

        response = InputReceiver().validate_input('Бажаєте продовжити роботу з цим рядом? [Y\\n]', 'очікується y або n', 
                                lambda x: x in ['Y', 'y', 'N', 'n'], key=3)
        if response in ['N', 'n']:
            break
...
def run():
    ...
    if DW_series:
        ...
        print(f'Розраховане за формулою: {dw_stat:.6f} \t Істинне (statsmodels): {durbin_watson(DW_series):.6f}')
    ...
    while True:
        try:
            series = InputReceiver().read_file(f'Вкажіть шлях до файлу з часовим рядом для аналізу (за замовчуванням {series_file_path})', 
                                               file_path_override=series_file_path)
        except Exception:
             print(f"Помилка: Файл {series_file_path} не знайдено. Введіть шлях до ряду RTStl.txt вручну.")
        ...
        response = InputReceiver().validate_input('Бажаєте завантажити інший ряд для Частини 2? [Y\\n]', 'очікується y або n', 
                            lambda x: x in ['Y', 'y', 'N', 'n'], key=3)
        if response in ['N', 'n']:
            break
    ...
    eq_str += ' + '.join([f'{coefs[i+1]:.4f}*{feat}' for i, feat in enumerate(important_features)])
    ...
    # Звіт статистики
    stats_report(y, pred, n_params=len(coefs) - 1, p=len(coefs) - 1, coefs=coefs, name = 'Множинної регресії (МНК)')
